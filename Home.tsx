      ...
      <AnimatePresence>
        {showLogin && (
          <motion.section 
            ...
            className="bg-white rounded-2xl shadow-xl border border-slate-100 p-8 overflow-hidden"
          >
            <div className="text-center mb-8">
              <h2 className="text-3xl font-bold text-slate-900">{t('login_modal_title' as TranslationKey)}</h2>
              <p className="text-slate-500">{t('login_modal_desc' as TranslationKey)}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {roles.map((role) => (
                <button
                  ...
                  className="flex flex-col items-center p-6 rounded-xl border-2 border-slate-100 hover:border-indigo-500 hover:shadow-lg transition-all group bg-white relative overflow-hidden"
                >
                  <div className={`w-14 h-14 rounded-full flex items-center justify-center mb-4 ${role.color} shadow-md`}>
                    <role.icon className="w-7 h-7" />
                  </div>
                  <h3 className="font-bold text-slate-900 text-lg mb-1 text-center">{t((`role_${role.id}` as any) as TranslationKey)}</h3>
                  <p className="text-slate-500 text-xs mb-6 font-medium uppercase tracking-wide">{t((`role_desc_${role.id}` as any) as TranslationKey)}</p>
                  
                  <div className="w-full mt-auto">
                    <span className={`block w-full py-2 rounded-lg text-sm font-bold text-white transition-colors ${role.color} opacity-90 group-hover:opacity-100`}>
